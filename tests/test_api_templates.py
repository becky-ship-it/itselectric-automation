...
def test_create_template(client):
    resp = client.post(
        ...
        json={"subject": "Hello", "body_md": "<p>Hi</p>"},
    )
    assert resp.status_code == 200
    data = resp.json()
...
def test_create_template_conflict(client):
    client.post("/api/templates/dup-tmpl", json={"subject": "A", "body_md": "B"})
    resp = client.post("/api/templates/dup-tmpl", json={"subject": "A", "body_md": "B"})
    assert resp.status_code == 409


def test_get_template_after_create(client):
    client.post("/api/templates/get-test", json={"subject": "S", "body_md": "B"})
...
def test_update_template(client):
    client.post("/api/templates/upd-tmpl", json={"subject": "Old", "body_md": "Old"})
    resp = client.put("/api/templates/upd-tmpl", json={"subject": "New", "body_md": "New"})
    assert resp.status_code == 200
    assert resp.json()["subject"] == "New"


def test_update_template_not_found(client):
    resp = client.put("/api/templates/ghost", json={"subject": "X", "body_md": "Y"})
    assert resp.status_code == 404


def test_delete_template(client):
    client.post("/api/templates/del-tmpl", json={"subject": "D", "body_md": "D"})
